...
const CaptionNew = () => {
  ...
  useEffect(() => {
    // Create worker only once when component mounts
  ...
  async function submitButton(timerComplete) {
    try {
      ...
      if (timerComplete || numOfPlayersSubmitting === 0) {
        //if timer runs out or everyone votes
        let publishTimer = 0;
        ...
        console.log(
          "here line 221 numOfPlayersVoting:",
          numOfPlayersSubmitting
        );
        timeout();
      }
    } catch (error) {
  ...
  const handleNavigate = () => {
    console.log(
      "here in navigate 190, captionSubmitted, document hidden, time remaining",
      isCaptionSubmitted.current,
      document.hidden,
      localStorage.getItem("remaining-time")
    );
    ...
    if (document.hidden) {
      ...
      console.log(
        "current time, minimize time , their diff",
        currentTime,
        minimizeTime,
        currentTime - parseInt(minimizeTime)
      );
      let diff;
      ...
      console.log(
        "here diff is less that zero, isOutofSync",
        val,
        localStorage.getItem("isOutofSync")
      );
      if (val < -4) {
    ...
    if (isDeSync == "false") {
      ...
      console.log(
        "here 238, diff, minimizeTime",
        diff,
        minimizeTime,
        localStorage.getItem("minimize-time")
      );
      ...
      if (!userData.host) {
        ...
        if (!document.hidden) {
          ...
          console.log(
            "curr, minimize time, curr - minimize time",
            curr,
            minimizeTime,
            curr - parseInt(minimizeTime)
          );
          ...
          if (diff >= userData.roundTime + rem) {
            console.log(
              "274 diff, userData.roundTime,rem",
              diff,
              userData.roundTime,
              rem
            );
  ...
  useEffect(() => {
    subscribe((event) => {
      if (event.data.message === "Start Vote") {
        console.log(
          "getting called from subscribe 294, time --->",
          new Date().getTime(),
          captionSubmitted
        );
        handleNavigate();
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.hidden) {
        ...
        if (!captionSubmitted) {
          console.log(
            "here line 313 scaptionsubmited variable --->",
            captionSubmitted
          );
          if (!captionSubmitted) {
            console.log("Before posting message to worker", {
              userData,
              remainingTime,
              userCaption,
            });
            ...
            console.log("After posting message to worker");
          }
        }
        ...
        console.log(
          "332 page back online, timeremaining, diff",
          timeRemaining,
          diff,
          timeRemaining - diff
        );
        if (timeRemaining - diff < 0 || captionSubmitted) {
          ...
          console.log(
            "remainingTime, localhsot vvall 339",
            remainingTime,
            localStorage.getItem("remaining-time")
          );
